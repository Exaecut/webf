...
namespace webf {
void DOMStringMapPublicMethods::Release(DOMStringMap* dom_string_map) {
  dom_string_map->ReleaseAlive();
}
WebFValue<DOMStringMap, WebFPublicMethods> DOMStringMapPublicMethods::DynamicTo(webf::DOMStringMap* dom_string_map, webf::DOMStringMapType dom_string_map_type) {
  switch (dom_string_map_type) {
    case DOMStringMapType::kDOMStringMap: {
      WebFValueStatus* status_block = dom_string_map->KeepAlive();
      return WebFValue<DOMStringMap, WebFPublicMethods>(dom_string_map, dom_string_map->domStringMapPublicMethods(), status_block);
    }
    default:
      assert_m(false, ("Unknown DOMStringMapType " + std::to_string(static_cast<int32_t>(dom_string_map_type))).c_str());
      return WebFValue<DOMStringMap, WebFPublicMethods>::Null();
  }
}
}  // namespace webf
