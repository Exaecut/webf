// Generated by WebF TSDL, don't edit this file directly.
// Generate command: node scripts/generate_binding_code.js
// clang-format off
/*
 * Copyright (C) 2022-present The WebF authors. All rights reserved.
 */
#include "plugin_api/hybrid_router_change_event.h"
#include "plugin_api/event_target.h"
#include "plugin_api/exception_state.h"
#include "plugin_api/executing_context.h"
#include "core/dom/events/event.h"
#include "core/dom/events/custom_event.h"
#include "core/native/script_value_ref.h"
#include "core/dom/events/event_target.h"
#include "core/events/animation_event.h"
#include "core/events/close_event.h"
#include "core/events/focus_event.h"
#include "core/events/gesture_event.h"
#include "core/events/hashchange_event.h"
#include "core/events/input_event.h"
#include "core/events/intersection_change_event.h"
#include "core/events/pop_state_event.h"
#include "core/events/mouse_event.h"
#include "core/api/exception_state.h"
#include "core/events/pointer_event.h"
#include "core/events/transition_event.h"
#include "core/events/ui_event.h"
#include "core/dom/legacy/element_attributes.h"
#include "core/css/inline_css_style_declaration.h"
#include "core/css/computed_css_style_declaration.h"
#include "core/dom/legacy/bounding_client_rect.h"
#include "core/dom/dom_string_map.h"
#include "core/timing/performance_mark.h"
#include "core/dom/mutation_observer_registration.h"
#include "core/input/touch_list.h"
#include "core/input/touch.h"
#include "core/timing/performance_measure.h"
#include "core/events/promise_rejection_event.h"
#include "core/events/hybrid_router_change_event.h"
#include "core/events/error_event.h"
#include "core/events/message_event.h"
#include "plugin_api/hybrid_router_change_event_init.h"
namespace webf {
NativeValue HybridRouterChangeEventPublicMethods::State(HybridRouterChangeEvent* hybrid_router_change_event, SharedExceptionState* shared_exception_state) {
  auto value = hybrid_router_change_event->state();
  auto native_value = value.ToNative(hybrid_router_change_event->ctx(), shared_exception_state->exception_state, false);
  return native_value;
}
AtomicStringRef HybridRouterChangeEventPublicMethods::Kind(HybridRouterChangeEvent* hybrid_router_change_event) {
  auto value_atomic = hybrid_router_change_event->kind();
  return AtomicStringRef(value_atomic);
}
AtomicStringRef HybridRouterChangeEventPublicMethods::Name(HybridRouterChangeEvent* hybrid_router_change_event) {
  auto value_atomic = hybrid_router_change_event->name();
  return AtomicStringRef(value_atomic);
}
WebFValue<HybridRouterChangeEvent, HybridRouterChangeEventPublicMethods> ExecutingContextWebFMethods::CreateHybridRouterChangeEvent(ExecutingContext* context, ExceptionState& exception_state) {
  HybridRouterChangeEvent* obj = HybridRouterChangeEvent::Create(context, exception_state);
  WebFValueStatus* status_block = obj->KeepAlive();
  return WebFValue<HybridRouterChangeEvent, HybridRouterChangeEventPublicMethods>(obj, obj->hybridRouterChangeEventPublicMethods(), status_block);
};
}  // namespace webf
