...
#include "plugin_api/pop_state_event_init.h"
namespace webf {
NativeValue PopStateEventPublicMethods::State(PopStateEvent* pop_state_event, SharedExceptionState* shared_exception_state) {
  auto value = pop_state_event->state();
  auto native_value = value.ToNative(pop_state_event->ctx(), shared_exception_state->exception_state, false);
  return native_value;
}
WebFValue<PopStateEvent, PopStateEventPublicMethods> ExecutingContextWebFMethods::CreatePopStateEvent(ExecutingContext* context, ExceptionState& exception_state) {
  PopStateEvent* obj = PopStateEvent::Create(context, exception_state);
  WebFValueStatus* status_block = obj->KeepAlive();
  return WebFValue<PopStateEvent, PopStateEventPublicMethods>(obj, obj->popStateEventPublicMethods(), status_block);
};
}  // namespace webf
