...
#include "plugin_api/mouse_event_init.h"
namespace webf {
double MouseEventPublicMethods::Button(MouseEvent* mouse_event) {
  return mouse_event->button();
}
double MouseEventPublicMethods::ClientX(MouseEvent* mouse_event) {
  return mouse_event->clientX();
}
double MouseEventPublicMethods::ClientY(MouseEvent* mouse_event) {
  return mouse_event->clientY();
}
double MouseEventPublicMethods::OffsetX(MouseEvent* mouse_event) {
  return mouse_event->offsetX();
}
double MouseEventPublicMethods::OffsetY(MouseEvent* mouse_event) {
  return mouse_event->offsetY();
}
WebFValue<MouseEvent, MouseEventPublicMethods> ExecutingContextWebFMethods::CreateMouseEvent(ExecutingContext* context, const char* type, ExceptionState& exception_state) {
  AtomicString type_atomic = AtomicString(context->ctx(), type);
  MouseEvent* event = MouseEvent::Create(context, type_atomic, exception_state);
  WebFValueStatus* status_block = event->KeepAlive();
  return WebFValue<MouseEvent, MouseEventPublicMethods>(event, event->mouseEventPublicMethods(), status_block);
};
WebFValue<MouseEvent, MouseEventPublicMethods> ExecutingContextWebFMethods::CreateMouseEventWithOptions(ExecutingContext* context, const char* type, WebFMouseEventInit* init,  ExceptionState& exception_state) {
  AtomicString type_atomic = AtomicString(context->ctx(), type);
  std::shared_ptr<MouseEventInit> init_class = MouseEventInit::Create();
  ...
  MouseEvent* event = MouseEvent::Create(context, type_atomic, init_class, exception_state);
  WebFValueStatus* status_block = event->KeepAlive();
  return WebFValue<MouseEvent, MouseEventPublicMethods>(event, event->mouseEventPublicMethods(), status_block);
};
}  // namespace webf
