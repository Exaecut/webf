...
WebFValue<IntersectionChangeEvent, IntersectionChangeEventPublicMethods> ExecutingContextWebFMethods::CreateIntersectionChangeEventWithOptions(ExecutingContext* context, const char* type, WebFIntersectionChangeEventInit* init,  ExceptionState& exception_state) {
  AtomicString type_atomic = AtomicString(context->ctx(), type);
  std::shared_ptr<IntersectionChangeEventInit> init_class = IntersectionChangeEventInit::Create();
  init_class->setIntersectionRatio(init->intersection_ratio);
  ...
  IntersectionChangeEvent* event = IntersectionChangeEvent::Create(context, type_atomic, init_class, exception_state);
  WebFValueStatus* status_block = event->KeepAlive();
  return WebFValue<IntersectionChangeEvent, IntersectionChangeEventPublicMethods>(event, event->intersectionChangeEventPublicMethods(), status_block);
};
}  // namespace webf
