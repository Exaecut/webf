...
namespace webf {
NativeValue PerformanceMeasurePublicMethods::Detail(PerformanceMeasure* performance_measure, SharedExceptionState* shared_exception_state) {
  auto value = performance_measure->detail();
  auto native_value = value.ToNative(performance_measure->ctx(), shared_exception_state->exception_state, false);
  return native_value;
}
}  // namespace webf
